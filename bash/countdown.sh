...
function handle-interrupt {
    echo "You are not allowed to interrupt the count. Resetting count to maximum."
    sleepCount=$numberOfSleeps
}

function handle-quit {
    echo "Congratulations! You found the secret to getting out of the script. Exiting immediately."
    exit 0
}
...
# Set traps for INT and QUIT signals
trap 'handle-interrupt' INT
trap 'handle-quit' QUIT
